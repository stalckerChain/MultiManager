...
describe('GET /api/matrix', () => {
  ...
  beforeEach(() => {
    ...
    // Seed projects in DB — including disabled_proj as inactive
    const projects = createProjectQueries(db);
    projects.sync([
      { name: 'concrete', display_name: 'Concrete', is_active: true, default_config: JSON.stringify({ accounts: ['001-050'] }) },
      { name: 'allscale', display_name: 'AllScale', is_active: true, default_config: JSON.stringify({ accounts: ['001-050'] }) },
      { name: 'disabled_proj', display_name: 'Disabled', is_active: false, default_config: '{}' },
    ]);
