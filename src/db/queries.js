  ...
  return {
    ...
    update(id, data) {
      update.run(
        ...
        data.platform || null,
        data.user_agent !== undefined ? data.user_agent : null,
        data.screen_resolution !== undefined ? data.screen_resolution : null,
        data.hardware_cores !== undefined ? data.hardware_cores : null,
        data.hardware_memory !== undefined ? data.hardware_memory : null,
        data.fingerprint_seed !== undefined ? data.fingerprint_seed : null,
...
function createProxyQueries(db) {
  ...
  const findByHostPort = db.prepare('SELECT * FROM proxies WHERE LOWER(TRIM(host)) = ? AND port = ?');
