...
- **[Feature]** Корректное завершение браузера в MM-automation во всех сценариях (штатное, ошибка проекта, `KeyboardInterrupt`/остановка CLI, отмена automation-run, недоступность MM API). Остановка профиля в MM-режиме выполняется только через `POST /api/browser/:id/stop` (CDP graceful shutdown): `BaseBrowser.close()` идемпотентен (флаг `_closed`), сначала `stop_browser(profile_id)`, затем остановка Playwright CDP-сессии. Отмена run (`POST /api/runs/:id/cancel`) сначала инициирует остановку браузеров профилей через MultiManager lifecycle (`stopProfile` → `gracefulCloseBrowser`), затем принудительно завершает Python-процессы; двойная остановка профиля блокируется существующим `stoppingProfiles`. stAuto0 в MM-режиме не выполняет глобальный `kill_chrome_processes()` (`taskkill /F /IM chrome.exe|node.exe`) из обработчиков `KeyboardInterrupt`/`SystemExit`/исключения; legacy-режим сохраняет прежний cleanup. API-контракт endpoint-ов, схема БД, зависимости и версия не менялись. Версия — **1.5.1**.
