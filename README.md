...
- **[Feature] Shift-выделение диапазона в Automation → Matrix** — в `AutomationMatrix.vue` добавлен `lastSelectedKey` и обработчик `handleCellClick` с копированием состояния anchor на весь прямоугольный диапазон. Обычный клик переключает одну доступную ячейку и запоминает anchor; `Shift+клик` копирует состояние anchor (включено/выключено) на диапазон между anchor и текущей ячейкой по `filteredProfiles` и активным проектам, с фильтрацией `allowed_profile_ids`, валидацией anchor по текущему отображению и поиску, без изменения API/БД. `a-checkbox` обёрнут в `<span @click>` и `@change` удалён для исключения двойного переключения. Тесты `gui-matrix-selection.test.js` — 12 сценариев диапазона (границы, обратное направление, `allowed_profile_ids`, Shift без anchor, невалидный anchor после фильтрации, копирование выключения). Версия — **1.5.1**.

- **[Feature] Retry и дублирование runs** — в `Automation → Runs` для каждого run доступны две операции независимо от статуса: «Повторить невыполненные» (копирует только задачи со `status != 'success'`) и «Дублировать» (копирует все задачи). Новый run создаётся `pending` с `pending` задачами, без переноса `exit_code`/`log_file_path`/`attempts`/`error_message`/`started_at`/`completed_at`; исходный run не меняется и не запускается автоматически. Форма модалки предзаполняется `Run YYYY-MM-DD HH:mm` и `parallel_limit=1` (редактируется, пустое имя → авто-имя). Создание атомарно в транзакции; при пустом наборе `retry`/`duplicate` → `400` без частичных данных, несуществующий run → `404`. Новые endpoint-ы `POST /api/runs/:id/retry` и `POST /api/runs/:id/duplicate` (валидация `runCreateSchema`, `parallel_limit` 1..50). GUI — `AutomationRuns.vue` (кнопки с `@click.stop`, единый modal), `stores/automation.js` (`retryRun`/`duplicateRun`), i18n `ru/en/zh`. Версия — **1.5.1**.

- **[FIX] Надёжная синхронизация popup Zerion в Multi-Control** — устранено появление обычных вкладок `chrome-extension://...` вместо нативных popup-окон Zerion при задержке открытия popup в slave (нативный target антидетекта не приходит через CDP WS, master обнаруживается через HTTP `/json` polling 300мс). Сохранена синхронизация мыши и поведение для обычных `http(s)` вкладок, включая `_blank` и `window.open`.
  - `src/api/multi-control.js`: локальные helper `getChromeExtensionInfo`/`getZerionRuntimeIdForProfile`/`classifyMasterUrl` — распознаётся только `chrome-extension://`, runtime ID через `resolveRuntimeId(extPath, profileDir)` из `profile.extensions` конкретного профиля (`getBrowserDataDir`), сопоставление `extension ID` + `pathname` (query/hash игнорируются), любой `chrome-extension://` без совпадения ID/пути не считается Zerion-popup (ошибки чтения профиля/JSON/`resolveRuntimeId` — не классифицировать как popup, лог без секретов, кэш `runtimeIdCache` 5с); `_findNativeSlaveTab(slaveId, expectedUrl)` — для Zerion polling 2–3с шаг 200мс (`POPUP_INITIAL_WAIT_MS`/`POPUP_POLL_INTERVAL_MS`) через `getHttpTabs`, исключает уже замапленные `tabMapping` для slave, принимает только `page` чей URL совпадает с Zerion ID конкретного slave и `pathname` master-popup (случайная `http(s)` вкладка не принимается), для `http(s)` сохранено короткое ожидание 2×150мс; `syncNewMasterTab` — трёхветочная классификация (подтверждённый Zerion-popup — длительное ожидание + reconciliation без `createTab`, нераспознанный `chrome-extension://` — warning без `createTab`, `http(s)` — `createTab`→`attach`→`map`), параллельное ожидание для slave через `Promise.all`, долгое ожидание не блокирует `discoverActiveTab`/`/os-keyboard` Enter (Zerion `sync` fire-and-forget); reconciliation через `pendingPopupReconciliations` (проверка на `discoverActiveTab` 300мс + 5.5с после таймаута для 5+с задержек под нагрузкой), `attach` + in-place remap `tabMapping.get(masterTargetId).set(slaveId, newTargetId)` без `unmap`+`map` (сохраняет `tabIndex`), cleanup закрывает только явно ошибочный fallback (`createdPopupFallbackTargets` или URL-совпадение ID+pathname), идемпотентно, очистка в `/stop`/`/slave/remove` вместе с `pendingSync`/`attachedMasterTabs`; `onNewTab`/`onTabAttached` для slave не маппят `chrome-extension://` target по `tabIndex`; `tabIndex` для обычных табов сохранён.
  - Тесты: `tests/unit/multi-control-api.test.js` (+14 тестов: delayed popup >300мс без `createTab`, Enter не блокируется <1с в т.ч. полная цепочка `/os-keyboard`, случайная `http` не принимается за popup, timeout без вкладки, нераспознанный `chrome-extension://` после ошибки `resolveRuntimeId` без `createTab`, обычный `_blank` с `createTab`, reconciliation после 5+с, late popup заменяет ошибочный fallback с `closeTarget`, idempotency, wrong ID/pathname не мапится, in-place remap сохраняет `tabIndex`, parallel wait).
  - API-контракт, схема БД, зависимости и версия не менялись. Версия — **1.5.1**.

- **[Feature]** Исправлен ввод в «Синхронизаторе» (Multi-Control). При вводе printable-символа в master символ больше не дублируется в slave: `keyDown` для печатных клавиш не отправляется через `Input.dispatchKeyEvent`, символ идёт единственным текстовым каналом `charInput → Input.insertText` (корректные символы любой раскладки, Shift, AltGr, dead keys). Ввод из slave-окна (и любых других окон) больше не рассылается: каждое событие клавиатуры (`keyDown`/`keyUp`/`charInput`) несёт PID foreground-окна-источника, а `/api/multi-control/os-keyboard` рассылает в slave только события из master-профиля (`pq.getById(masterId)?.pid`). Управляющие символы C0/DEL (`\b`, `\t`, `\r`, `\x7f`), которые `ToUnicodeEx` возвращает для Backspace/Tab/Enter/Delete, текстом не считаются — Backspace/Delete/Tab/Enter форвардятся как клавиши (в slave нет «квадратиков»), управляющие key events отправляются с очищенным `text`. Browser shortcuts (Ctrl+T/W/N), Ctrl+1, Meta, обычный Alt, Shift и AltGr сохраняют прежнее поведение. API-контракт, схема БД, зависимости и версия не менялись. Версия — **1.5.1**.

- **[Feature]** Снижена задержка движения курсора в «Синхронизаторе» (Multi-Control) и исправлена синхронизация прокрутки. Входящие `mousemove` от мастера троттлятся (интервал 16 мс, стратегия `latest-event-wins`, только движение — клики/клавиатура/scroll не задерживаются); параметры MouseSmoother адаптируются к числу slave (1–2: `stepInterval=8/maxPoints=60`, 3–4: `12/40`, 5+: `16/30`, `moveSpeed=5` стабилен); устаревшие точки пропускаются без повторного пересчёта траекторий. Исправлена формула координат slave — без двойного вычитания scroll. Прокрутка синхронизируется по фактическому документному скроллу мастера: authoritative источник — событие `window.scroll` (абсолютные `window.scrollX/scrollY` после изменения прокрутки, частые события коалесцируются), а не wheel и не сумма дельт; wheel — только диагностика и не запускает скролл. Применение в slave — через CDP `Runtime.callFunctionOn('window.scrollTo(x, y)')` с `executionContextId` сессии (без fallback `Runtime.evaluate`); неуспешное применение фиксируется диагностическим счётчиком, состояние slave фиктивным значением не подменяется; после стабилизации серии фактический scroll slave перечитывается. `Runtime.enable` отправляется для всех сессий (включая slave с `enableInput: false`). API-контракт, схема БД, зависимости и версия не менялись. Версия — **1.5.1**.

- **[Feature]** Отображение имени профиля в окнах синхронизатора. В `Window Preview` и `Detected Windows` вместо системного заголовка окна Chromium (например, `Untitled - Chromium`) теперь показывается имя профиля из MultiManager. Для Windows при формировании `GET /api/window-arranger/windows` строится lookup `PID -> имя` по запущенным профилям (`profiles.name`, при пустом имени — `profiles.id`), PID окна из Win32-скрипта сопоставляется с lookup, и ответ окна содержит `windowTitle` (исходный системный заголовок) и `name` (имя профиля; при отсутствии сопоставления — `windowTitle`). Поле `id` остаётся HWND-дескриптором, поэтому `Focus`, `Grid` и `Cascade` не изменены; новые поля `pid`/`profileId`/`profileName` в ответ не добавляются. Linux/macOS (fallback системного имени) и фильтры crash/restore и минимального размера не изменены. API-контракт, схема БД, зависимости и версия не менялись. Версия — **1.5.1**.

- **[Feature]** Информационная вкладка профиля при запуске. При старте профиля (`POST /api/browser/:id/start`) в браузере открывается вкладка с локальной HTML-страницей `http://127.0.0.1:<port>/profile-info/<profileId>`, заголовок которой равен имени аккаунта. Если после стартовых операций (автологин/нормализация вкладок) осталась пустая вкладка `about:blank` — страница открывается в ней (без создания лишней вкладки), иначе создаётся новая. Публичный loopback-endpoint `GET /profile-info/:profileId` (подключён до `authMiddleware`, без авторизации и rate limiter) возвращает только 8 согласованных полей: имя, email, `wallet_evm_address`, `wallet_sol_address`, X username, Discord username, IP и локацию прокси; отсутствующие значения — placeholder «Не указано», неизвестный профиль — 404. Секреты (пароли, токены, email, proxy credentials, fingerprint seed) не включаются в HTML, JSON и логи; URL вкладки и текст ошибки CDP не логируются. Порт берётся из `req.socket.localPort` start-запроса — работает при нестандартном порте MultiManager; сбой открытия вкладки не переводит запущенный профиль в failed (записывается warning). Режимы ручного и automation/MM запуска (включая legacy-`stAuto0`) не изменены. API-контракт, схема БД, зависимости и версия не менялись. Версия — **1.5.1**.
...
- **[Feature]** Автологин кошелька при ручном запуске профиля. При вызове `POST /api/browser/:id/start` **без `run_id`** (ручной запуск с главной страницы) выполняется preflight: если у профиля заданы непустые `wallet_evm_address` и `wallet_password`, браузер автоматически логинится в Zerion; перед автологином и после него (включая ошибку) все page-вкладки закрываются и остаётся одна чистая вкладка `about:blank`. Если хотя бы одно wallet-поле отсутствует — автологин не выполняется, вкладки всё равно нормализуются. Ошибка автологина не останавливает браузер: пишется в профильный лог без пароля, EVM-адреса и URL. Automation-запуск с `run_id` ручной автологин не вызывает. Операция нормализации вкладок (`resetToSingleBlankTab`) реализована в `src/cdp/profile-tabs.js` в одном CDP-сеансе: `devtools://`-вкладки не закрываются, WebSocket закрывается в `finally`, URL не логируются; после закрытия вкладок операция дожидается их фактического удаления (опрос `Target.getTargets`), т.к. `Target.closeTarget` подтверждает только приём команды, а уничтожение таргета асинхронно. Версия — **1.5.1**.
