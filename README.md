...
> **Фазы Roadmap:** Ф1–Ф4 ✅, Ф5 ✅, **Ф6 ✅** (планировщик + spawn + терминал).
> **Интеграция stAuto0:** ФГ ✅, ФА ✅, ФБ ✅, **ФВ ✅** (Wallet Factory), **ФД ✅** (MCP).
...
- **Built-in Terminal:** xterm.js + IPC-мост для tail -f логов задач (powershell Get-Content / tail). Привязан к `log_file_path` в `task_executions` — кнопка "View Log" в GUI.
> **PowerShell invocation:** все вызовы PowerShell (`Get-RunningWindows`, `Move-Window`, `Set-WindowFocus`) используют `-EncodedCommand` (Base64 UTF‑16LE) вместо temp-файлов, что bypassит Execution Policy и ASR-правила.
...
- `app.db` — SQLite (WAL). Профили (30 колонок, AES-256-GCM), прокси, куки, задачи (tasks/task_executions), system_config.
