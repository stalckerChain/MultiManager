...
> **Полная спецификация:** [TS.md](./TS.md) (MultiManager v1.3.1) + [TS_INTEGRATION.md](./TS_INTEGRATION.md) (stAuto0 интеграция).
> **Фазы Roadmap:** Ф1–Ф4 ✅, Ф5 ✅, Ф6 ✅, **Ф7 ✅ (Automation Matrix)** — подробный план [TASK.md](./TASK.md).

## Что нового в v1.3.2 (Hotfix)

### Исправление: обнаружение закрытия браузера через "крестик"

Проблема: при закрытии браузера кнопкой "крестик" самого браузера (не из приложения), статус в GUI оставался "running".

**Корневые причины:**
1. `child.on('exit')` для detached+unref'd процессов может не сработать на Windows
2. Фронтенд не перезапрашивал данные после переподключения WebSocket — потерянные broadcast'ы терялись навсегда
3. Отсутствовал периодический опрос статуса как страховочный механизм

**Исправления:**
- `gui/src/renderer/composables/useWebSocket.js` — `fetchAll()` при переподключении WebSocket
- `src/api/browser.js` — `isProcessAlive()` с валидацией pid и поддержкой Windows-кодов ошибок
- `gui/src/renderer/views/Profiles.vue` — периодический опрос статуса каждые 10 сек (только при наличии running профилей)
...
- **WebSocket:** Реалтайм-статусы профилей. Exponential backoff (1→2→4→8 сек). Автоматический re-fetch при переподключении. Периодический опрос статуса (10 сек) как страховка от потери broadcast'ов.
...
└── tests/                    # Vitest (710+ тестов, 45 файлов)
    ├── unit/                 # 34 файла: auth, proxy, fingerprint, typing, crypto, pty, automation, matrix-selection, websocket-reconnect, etc.
