...
- **[EXP] Устранён штраф «Скрытый режим» в BrowserScan.** BrowserScan видел «Скрытый режим: Да (штраф -10%)» из-за нормализованного CloakBrowser storage quota. В запуск добавлен документированный флаг `--fingerprint-storage-quota=10240`; `--unlimited-storage` не используется. Гипотеза подтверждена на том же профиле и прокси — штраф «Скрытый режим» исчез. Seed, proxy и остальные параметры запуска не изменены. ✅ `src/api/browser.js`, `tests/unit/browser-start-await.test.js`
