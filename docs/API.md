...
ручного автологина, перед ответом) создаётся новая вкладка с URL
...
Страница доступна без авторизации только на loopback (см. раздел
«Информация о профиле»). Ошибка создания вкладки не останавливает уже успешный
запуск: она логируется безопасно (только `profileId` и категория ошибки),
URL вкладки не логируется.
