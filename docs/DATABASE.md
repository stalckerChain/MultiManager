...
| `wallet_password` | TEXT | Пароль кошелька (может быть пустым/NULL) |
...
**Хранение секретов:** Поля профилей (`email_password`, `twitter_password`, `twitter_auth_token`, `discord_password`, `discord_token`, `wallet_password`) и прокси (`username`, `password`) хранятся и читаются как обычные `TEXT`-значения без шифрования. Значения, ранее сохранённые в формате `aes-256-gcm:...`, автоматически не мигрируются и не преобразуются — query-слой отдаёт их как есть до внешнего восстановления.
...
| `location` | TEXT | Локация прокси, формат `DE(Germany)` (определяется при check) |
| `created_at` | DATETIME | Дата создания |
| `updated_at` | DATETIME | Дата обновления |

**Индексы:**
- `idx_proxies_host_port` — поиск по host:port (обычный индекс поиска; уникальность пары `host:port` обеспечивается прикладной проверкой на уровне API, а не этим индексом)
...
Очередь одноразового импорта куки. Записи применяются в браузер после запуска
профиля через CDP `Network.setCookies`; после подтверждения применения через
`Network.getAllCookies` (по ключу `(domain, path, name)`) подтверждённые записи
удаляются. При ошибке CDP-инъекции записи сохраняются для повторной попытки.
Уже применённые куки в этой таблице не хранятся.
