...
- **[FEAT] Информационная вкладка профиля при запуске.**
  ...
  - Тесты: `tests/unit/profile-info.test.js` (новый, 11 тестов: 200/404, отсутствие секретов, escaping, корректный `<title>`), `tests/unit/browser-profile-info-tab.test.js` (новый, 19 тестов: `Target.createTarget` и навигация существующей `about:blank`-вкладки через `Page.navigate` для ручного и automation/MM запуска, `req.socket.localPort`, безопасные логи, отсутствие хардкода 3000); opt-in интеграционный тест в `tests/integration/profile-launch.test.js` (реальный CloakBrowser, пропускается в обычном `npm test`).
