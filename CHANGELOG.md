...
- **[FEAT] Информационная вкладка профиля при запуске.**
  При старте профиля (`POST /api/browser/:id/start`) в конце запуска создаётся новая вкладка с локальной HTML-страницей `http://127.0.0.1:<MultiManager-port>/profile-info/<profileId>`, заголовок которой равен имени аккаунта.
  ...
  - `src/api/browser.js`: создание вкладки через CDP `Target.createTarget` в самом конце старта — после `loadExtensionsViaCDP` и `runManualAutologin`, непосредственно перед `res.json`; порт — из `req.socket.localPort` start-запроса (без передачи через `spawnBrowserWithCdp` и без хардкода 3000); явная проверка `cdpPort`; сбой CDP не ломает успешный запуск (warning); безопасный лог только `profileId` + категория (`Target.createTarget`), URL и `err.message` не логируются.
  ...
  - Тесты: `tests/unit/profile-info.test.js` (новый, 11 тестов: 200/404, отсутствие секретов, escaping, корректный `<title>`), `tests/unit/browser-profile-info-tab.test.js` (новый, 15 тестов: `Target.createTarget` с URL информационной страницы для ручного и automation/MM запуска, `req.socket.localPort`, безопасные логи, отсутствие хардкода 3000); opt-in интеграционный тест в `tests/integration/profile-launch.test.js` (реальный CloakBrowser, пропускается в обычном `npm test`).
  ✅ `src/api/profile-info.js` (новый), `src/core/app.js`, `src/api/browser.js`, `tests/unit/profile-info.test.js` (новый), `tests/unit/browser-profile-info-tab.test.js` (новый), `tests/integration/profile-launch.test.js`, `docs/API.md`, `docs/API.en.md`, `docs/API.zh.md`, `TS.md`, `README.md`
