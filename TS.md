...
### 4.1. Генератор случайных отпечатков (Fingerprint Generator)
...
### 4.4. Логика синхронизатора (Multi-Control) — v0.13.0
...
### 4.6. Менеджер расширений (Extensions Manager)
...
### 4.7. Управление окнами (Window Arranger)
...
### 4.8. Очистка дискового пространства
...
### 4.9. Кроссплатформенный контроль процессов (Anti-Zombie)
...
### 4.10. Пакетный импорт/экспорт (Migration Wizard) — В РАЗРАБОТКЕ
- Перенос профилей, прокси и куки из AdsPower и Dolphin{anty} через их локальные API

### 4.11. Облачная синхронизация (Cloud Sync) — В РАЗРАБОТКЕ
- Резервное копирование профилей через Google Drive / S3 / Telegram Bot / локальную папку
...
## 1. Технологический стек и архитектура интерфейса
...
### 2.1. Экран «Менеджер профилей» (Profile Manager)
...
### 3.2. Автозапуск Core и обработка конфликтов портов
